...
const C = {
  bg: '#0a0818',
  bgCard: 'rgba(20, 18, 40, 0.7)',
  border: 'rgba(139, 110, 220, 0.15)',
  borderHover: 'rgba(139, 110, 220, 0.4)',
  text: '#f0e6ff',
  textDim: '#a090c0',
  textMuted: '#6b5b8a',
  gold: '#e8c547',
      ...
      for (let i = 0; i < particles.length; i++) {
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (d < 120 && ctx) {
            ...
            ctx.strokeStyle = `rgba(139, 110, 220, ${0.08 * (1 - d / 120)})`;
            ctx.lineWidth = 0.5;
            ctx.stroke();
          }
        }
...
/* ─── Invention Card ─── */
function InventionCard({ node, onClick, selected }: { node: Node; onClick: () => void; selected?: boolean }) {
  return (
    <motion.div
      whileHover={{ y: -6, scale: 1.02 }}
      whileTap={{ scale: 0.97 }}
      onClick={onClick}
      className={`relative cursor-pointer rounded-2xl overflow-hidden border transition-all duration-300 flex-shrink-0 w-40 sm:w-48 ${
        selected ? 'border-[#e8c547] shadow-lg shadow-[#e8c547]/20' : 'border-[rgba(139,110,220,0.15)] hover:border-[rgba(139,110,220,0.4)]'
      }`}
    ...
    >
      <div className="aspect-square relative overflow-hidden">
        {node.imageUrl ? (
          ...
          <div className="w-full h-full bg-gradient-to-br from-[#1a1530] to-[#2a2050] flex items-center justify-center">
            <Sparkles className="w-8 h-8 text-[#6b5b8a]" />
          </div>
        )}
        <div className="absolute inset-0 bg-gradient-to-t from-[#0a0818] via-transparent to-transparent" />
        <div className="absolute top-2 left-2">
          <span className="text-[10px] px-2 py-0.5 rounded-full bg-[#0a0818]/80 text-[#e8c547] font-bold border border-[#e8c547]/30">
            {node.year > 0 ? node.year : `${Math.abs(node.year)} BC`}
          </span>
        </div>
        {selected && (
          <div className="absolute top-2 right-2 w-6 h-6 rounded-full bg-[#e8c547] flex items-center justify-center">
            <Sparkles className="w-3 h-3 text-[#0a0818]" />
          </div>
        )}
      </div>
      <div className="p-3">
        <h3 className="text-sm font-bold text-[#f0e6ff] truncate">{node.name}</h3>
        <p className="text-[11px] text-[#6b5b8a] mt-0.5 truncate">{node.place}</p>
      </div>
    </motion.div>
  );
}
...
/* ─── Score Ring ─── */
function ScoreRing({ label, value, color, icon: Icon, delay }: { label: string; value: number; color: string; icon: any; delay: number }) {
  const r = 28;
  const circ = 2 * Math.PI * r;
  const pct = value / 100;
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay }} className="flex flex-col items-center gap-2">
      ...
      <div className="text-center">
        ...
        <div className="text-[10px] text-[#6b5b8a] uppercase tracking-wider">{label}</div>
      </div>
    </motion.div>
  );
}
...
/* ─── Path Node ─── */
function PathNode({ node, edge, index, total }: { node: Node; edge?: Edge; index: number; total: number }) {
  return (
    <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.12 }} className="flex items-start gap-4">
      <div className="flex flex-col items-center flex-shrink-0">
        <div className="w-12 h-12 rounded-xl overflow-hidden border-2 border-[rgba(139,110,220,0.3)]">
          {node.imageUrl ? (
            ...
            <div className="w-full h-full bg-[#1a1530] flex items-center justify-center text-[#6b5b8a] text-xs font-bold">{index + 1}</div>
          )}
        </div>
        {index < total - 1 && (
          <div className="w-0.5 h-10 bg-gradient-to-b from-[rgba(139,110,220,0.3)] to-[rgba(139,110,220,0.05)] mt-1" />
        )}
      </div>
      <div className="flex-1 pt-1">
        <div className="flex items-center gap-2">
          <h4 className="text-base font-bold text-[#f0e6ff]">{node.name}</h4>
          <span className="text-[10px] text-[#6b5b8a]">{node.year > 0 ? node.year : `${Math.abs(node.year)} BC`}</span>
        </div>
        {edge && (
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: index * 0.12 + 0.2 }}
            className="text-sm text-[#a090c0] mt-1 italic leading-relaxed">
            {edge.label}
            {edge.surpriseFactor > 0.6 && (
              <span className="ml-2 text-xs text-[#e8c547] font-medium">✨ {Math.round(edge.surpriseFactor * 100)}</span>
            )}
          </motion.p>
        )}
      </div>
...
export default function App() {
  ...
  const handleSearch = useCallback(async () => {
    ...
    setIsSearching(true);
    setPaths([]); setSelectedPath(null); setNarrative('');

    if (apiAvailable) {
      try {
        const res = await api.findPaths(startId, endId, minSteps, maxSteps);
        const p = res.paths.map(r => r.nodes);
        setPaths(p);
        if (p.length) selectPath(p[0], res.paths[0].edges, res.paths[0].scores);
      } catch {
        const f = findPaths(startId, endId, minSteps, maxSteps);
        setPaths(f); if (f.length) selectPath(f[0]);
      }
    } else {
      setTimeout(() => {
        const f = findPaths(startId, endId, minSteps, maxSteps);
  ...
  const handleRandom = useCallback(() => {
    ...
    setStartId(inventions[i1].id);
    setEndId(inventions[i2].id);
    setTimeout(() => {
      setIsSearching(true);
      setPaths([]); setSelectedPath(null); setNarrative('');
      setTimeout(() => {
        const f = findPaths(inventions[i1].id, inventions[i2].id, minSteps, maxSteps);
        setPaths(f); if (f.length) selectPath(f[0]);
        setIsSearching(false);
      }, 600);
    }, 100);
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
          ...
          <div className="flex items-center justify-center gap-3 mb-5">
            <button onClick={() => setPickerMode('from')}
              ...
              {startId && getNode(startId)?.imageUrl ? (
                <img src={getNode(startId)?.imageUrl} alt="" className="w-10 h-10 rounded-lg object-cover" />
              ) : <div className="w-10 h-10 rounded-lg bg-[#1a1530] flex items-center justify-center"><Compass className="w-5 h-5 text-[#6b5b8a]" /></div>}
              <div className="text-left">
                <div className="text-[10px] text-[#6b5b8a] uppercase tracking-wider">From</div>
                <div className="text-sm font-semibold text-[#f0e6ff]">{startId ? getNode(startId)?.name : 'Pick...'}</div>
              </div>
            </button>
            ...
            <button onClick={() => setPickerMode('to')}
              ...
              {endId && getNode(endId)?.imageUrl ? (
                <img src={getNode(endId)?.imageUrl} alt="" className="w-10 h-10 rounded-lg object-cover" />
              ) : <div className="w-10 h-10 rounded-lg bg-[#1a1530] flex items-center justify-center"><Compass className="w-5 h-5 text-[#6b5b8a]" /></div>}
              <div className="text-left">
                <div className="text-[10px] text-[#6b5b8a] uppercase tracking-wider">To</div>
                <div className="text-sm font-semibold text-[#f0e6ff]">{endId ? getNode(endId)?.name : 'Pick...'}</div>
              </div>
            </button>
          </div>
          ...
          <div className="flex items-center justify-center gap-6 mb-5">
            {[
              { label: 'Min', val: minSteps, set: setMinSteps, min: 3, max: 8 },
              { label: 'Max', val: maxSteps, set: setMaxSteps, min: 5, max: 15 },
            ].map(s => (
        ...
        <AnimatePresence>
          {pickerMode && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              className="fixed inset-0 z-50 bg-[#0a0818]/95 backdrop-blur-sm flex items-center justify-center p-4"
              onClick={() => setPickerMode(null)}>
              <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
                className="w-full max-w-3xl max-h-[75vh] rounded-2xl border overflow-hidden flex flex-col"
                ...
                <div className="flex items-center justify-between p-4 border-b" style={{ borderColor: C.border }}>
                  ...
                  <button onClick={() => setPickerMode(null)} className="p-2 hover:bg-[rgba(139,110,220,0.1)] rounded-lg transition-colors">
                    <X className="w-5 h-5 text-[#6b5b8a]" />
                  </button>
                </div>
                <div className="overflow-y-auto p-4">
                  <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3">
                    {inventions.map(node => (
                      <InventionCard key={node.id} node={node}
                        selected={pickerMode === 'from' ? startId === node.id : endId === node.id}
                        onClick={() => {
                          if (pickerMode === 'from') setStartId(node.id);
                          else setEndId(node.id);
                          setPickerMode(null);
                        }} />
                    ))}
                  </div>
          ...
          {isSearching && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="text-center py-16">
              <div className="inline-flex items-center gap-2">
                {[0, 0.15, 0.3].map(d => (
                  <motion.div key={d} className="w-3 h-3 bg-[#e8c547] rounded-full"
          ...
          {selectedPath && !isSearching && (
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="space-y-6">
              ...
              <div className="rounded-2xl p-5 border flex justify-around flex-wrap gap-4" style={{ background: C.bgCard, borderColor: C.border }}>
                {[
                  { k: 'serendipity', label: 'Serendipity', icon: Sparkles, color: C.gold },
                  { k: 'curiosity', label: 'Curiosity', icon: Eye, color: C.violet },
                  { k: 'synchronicity', label: 'Sync', icon: Zap, color: C.cyan },
                  { k: 'fortuity', label: 'Fortuity', icon: Flame, color: C.red },
                  { k: 'materiality', label: 'Material', icon: Gem, color: C.emerald },
                ].map((s, i) => (
                  <ScoreRing key={s.k} label={s.label} value={scores[s.k as keyof typeof scores]} icon={s.icon} color={s.color} delay={0.3 + i * 0.08} />
                ))}
              </div>
              ...
              <div className="rounded-2xl p-5 sm:p-6 border" style={{ background: C.bgCard, borderColor: C.border }}>
                ...
                <div className="space-y-0">
                  {selectedPath.map((node, i) => (
                    <PathNode key={node.id} node={node} edge={selectedEdges[i]} index={i} total={selectedPath.length} />
                  ))}
                </div>
              </div>
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }}
                className="rounded-2xl p-5 border" style={{ background: C.bgCard, borderColor: C.border }}>
                <h3 className="text-sm font-bold text-[#e8c547] mb-3 flex items-center gap-2">
                ...
                <div className="rounded-2xl p-5 border" style={{ background: C.bgCard, borderColor: C.border }}>
                  ...
                  <div className="space-y-2">
                    {paths.slice(1, 4).map((path, i) => (
                      <button key={i} onClick={() => selectPath(path)}
                        className="w-full text-left rounded-xl p-3 border border-[rgba(139,110,220,0.1)] hover:border-[#8b5cf6]/50 transition-all"
                        style={{ background: 'rgba(10, 8, 24, 0.5)' }}>
                        <div className="flex items-center gap-1 flex-wrap text-xs text-[#a090c0]">
                          {path.map((n, j) => (
                            <span key={n.id} className="flex items-center">
                              {n.name}{j < path.length - 1 && <ArrowRight className="w-3 h-3 mx-1 text-[#4a4e8f]" />}
                            </span>
                          ))}
                        </div>
                      </button>
